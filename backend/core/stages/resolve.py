...
async def resolve_emails(
    ...
    emit(on_event, "eazyreach", "start", 0, f"Resolving emails for {len(contacts)} contacts")
    ...
    async def one(contact: Contact) -> None:
        ...
        async with sem:
            ...
            if contact.is_deliverable:
                resolved += 1
                emit(on_event, "eazyreach", "progress", resolved,
            ...
            if contact.linkedin_url:
                try:
                    ...
                    emit(on_event, "eazyreach", "skip", 0, f"{contact.display_name}: {exc}")
            ...
            if contact.is_deliverable:
                ...
                emit(on_event, "eazyreach", "skip", resolved,
                     f"{contact.display_name}: no deliverable email")

    await asyncio.gather(*(one(c) for c in contacts))
    emit(on_event, "eazyreach", "done", resolved, f"{resolved} deliverable emails")
    return contacts
